...
echo [INFO] Checking if database.expiration_date parameter exists...
docker exec %DB_HOST% psql -U %DB_USER% -d %DB_NAME% -t -c "SELECT COUNT(*) FROM ir_config_parameter WHERE key = 'database.expiration_date';" > temp_count.txt
set /p PARAM_COUNT=<temp_count.txt
del temp_count.txt

REM Clean up the count
set PARAM_COUNT=%PARAM_COUNT: =%

if "%PARAM_COUNT%"=="0" (
    echo [INFO] Parameter does not exist. Creating it...
    docker exec %DB_HOST% psql -U %DB_USER% -d %DB_NAME% -c "INSERT INTO ir_config_parameter (key, value, create_date, write_date) VALUES ('database.expiration_date', '%EXPIRATION_DATE%', NOW(), NOW());"
    if %errorlevel% equ 0 (
        echo [SUCCESS] Successfully created expiration date parameter!
    ) else (
        echo [ERROR] Failed to create expiration date parameter.
        pause
        exit /b 1
    )
) else (
    echo [INFO] Parameter exists. Updating it...
    ...
    if %errorlevel% equ 0 (
        echo [SUCCESS] Successfully updated expiration date parameter!
    ) else (
        echo [ERROR] Failed to update expiration date parameter.
        pause
        exit /b 1
    )
)
...
if "%CURRENT_VALUE%"=="%EXPIRATION_DATE%" (
    ...
    echo [SUCCESS] Odoo will now show expiration date as: %EXPIRATION_DATE%
) else (
...
echo [INFO] The subscription warning should no longer appear in your Odoo interface.
echo.
pause
